...
function Post(props) {
  ...
  const commentCount =
    props.post.comments === [] ? 0 : props.post.comments.length;
  ...
  function handleComment() {
    console.log(comment);
    fetch(config.BACKEND_URL + "/api/comment", {
      method: "POST",
      headers: {
  ...
  function handleLike() {
    fetch(config.BACKEND_URL + "/api/like", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
      body: JSON.stringify({ post: props.post, me: props.me }),
    })
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
    ...
    <div className="post">
      <div className="post-header">
        ...
        <div className="post-header-details">
          <a href={"/user/" + props.post.authorId._id}>
            {firtstName + "  " + lastName + " @" + username}
          </a>
          <h3>16:31</h3>
        </div>
      </div>
      <div className="post-body">
        ...
        {image !== "" ? (
          <div className={`post-body-img ${check === false ? "" : "expanded"}`}>
            <img src={config.BACKEND_URL + "/posts/" + image} />
          </div>
        ) : (
          <div />
        )}
        ...
        <div className="post-body-actions">
          ...
          {image !== "" ? (
            <div className="expand">
              <img
                src={check === false ? expand : collapse}
      ...
      <div className="post-comments">
        ...
        {commentsState.map((value, key) => (
          <Comment key={key} user={value} />
        ))}
      </div>
    </div>
  );
